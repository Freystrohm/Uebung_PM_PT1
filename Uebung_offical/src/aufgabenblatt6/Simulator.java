...
public class Simulator
{
	...
	public Simulator()
	{
		for (int i = 0; i < listeSendungen.length; i++)
		{
			if (Math.random() > 0.5)
			{
				listeSendungen[i] = paketErstellen(
						stadt[(int) (stadt.length * Math.random())],
						stadt[(int) (stadt.length * Math.random())]);
			}
			else
			{
				listeSendungen[i] = briefErstellen(
						stadt[(int) (stadt.length * Math.random())],
						stadt[(int) (stadt.length * Math.random())]);
			}
		}
	...
	/**
	 * Erstellt eine Liste an Adressen und speichert diese in Array
	 * listeAdressen.
	 */
	public void adressenErstellen()
	{
		...
		{
			Adresse adresse = new Adresse("straße", (int) (Math.random() * 99),
	...
	/**
	 * Erstellt eine Liste von Personen und speichert diese in Array
	 * ListePersonen
	 */
	...
	/**
	 * Erstellt eine Liste von Sendungen und speichert diese in Array
	 * ListeSendungen
	 */
	public void sendungenErstellen()
	{
		...
		{
			Sendung paket = new Paket(listePersonen[21 - j],
					listePersonen[20 + j], (int) (Math.random() * 8));
	...
	public int getProzent(int zaehler)
	{
		...
		int gesamtDauer = offline.getSendungsTransportDauer(
				listeSendungen[zaehler].getSender().getAdresse().getOrt(),
				listeSendungen[zaehler].getEmpfaenger().getAdresse().getOrt());
		...
		while (eingabe != "schritt")
		{
			if (eingabe.equals("SCHRITT") || eingabe.equals("schritt"))
			{
				...
				for (int i = 0; i < listeSendungen.length; i++)
				{
					...
					if (listeSendungen[i].istAusgeliefert() == true)// getTransportDauer()
																	// <= 0)
					{
						sendungNeuerstellen(i);
					}
					System.out.println(
							toString(i) + "bei " + getProzent(i) + "%");
				}
				System.out
						.println("Bitte Kommando eingeben (SCHRITT oder ENDE)");
				eingabe = scan.next();
			}
	...
	public void sendungNeuerstellen(int zaehler)
	{
		...
		{
			listeSendungen[zaehler] = new Brief(
		...
		{
			listeSendungen[zaehler] = new Paket(
					listePersonen[(int) (Math.random() * 40)],
					listePersonen[(int) (Math.random() * 20)],
					(int) (Math.random() * 8));
			listeSendungen[zaehler].setStartZeitpunkt(schrittZaehler);
			System.out.println(toString(zaehler) + " erzeugt");
		}
